...
function dragInsideSameList(lists: Column[], payload: {
...
function dragBetweenLists(lists: Column[], payload: {
  droppableIdStart: string,
...
function getListByID(lists: Column[], listID: string): Column | undefined {
...
export const reducer = createReducer(
  ...
  on(CardActions.deleteCard, (state, payload) => {
    ...
    const listIndex = (lists as Column[]).findIndex(list => list.id === listID);
    const cardIndex = (lists as Column[])[listIndex].cards.findIndex(card => card.id === cardID);
